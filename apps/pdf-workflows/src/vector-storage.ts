...
export class VectorStorage {
  ...
  constructor() {
    const mongoUri =
      process.env.MONGO_URI || "mongodb://localhost:27017/apexflow";
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // Collections
      this.documentsCollection = this.db.collection<ProcessedDocument>(
        "processed_documents",
      );
      this.chunksCollection =
        this.db.collection<DocumentChunk>("document_chunks");
  ...
  private async createIndexes(): Promise<void> {
    try {
      ...
      try {
        ...
        logger.warn(
          "Vector search index creation failed (may not be supported in this MongoDB version)",
          {
            error: error.message,
          },
        );
  ...
  async storeProcessedDocument(
    ...
    console.log("VECTOR STORAGE: storeProcessedDocument method called!", {
      documentId,
    ...
    try {
      ...
      if (chunks.length > 0) {
        ...
        logger.warn(
          " No chunks to store (text too short or processing issue)",
          {
            documentId,
            extractedTextLength: extractedText.length,
          },
      ...
      console.log(
        "🔄 CONSOLE: About to update original document record - THIS SHOULD APPEAR!",
        {
          documentId,
          chunkCount: chunks.length,
          extractedTextLength: extractedText.length,
      ...
      try {
        ...
        logger.error(
          "CRITICAL: Successfully completed original document record update",
          {
            documentId,
            extractedTextLength: extractedText.length,
          },
  ...
  async getStats(): Promise<{
    ...
    try {
      const [totalDocuments, totalChunks, chunksWithEmbeddings] =
        await Promise.all([
   ...
   */
  private async updateOriginalDocumentRecord(
    documentId: string,
    processingResults: {
      extractedText: string;
      totalPages?: number;
    ...
    try {
      logger.info("Starting to update original document record", {
        documentId,
      });
      ...
      try {
        ...
        logger.info("Using ObjectId query for document update", {
          documentId,
          objectIdQuery: true,
        });
      } catch (objectIdError) {
        ...
        logger.info("Using string ID query for document update", {
          documentId,
          objectIdQuery: false,
      ...
      if (result.matchedCount > 0) {
        logger.info(
          "Successfully updated original document record with processing results",
          {
            documentId,
            extractedTextLength: processingResults.extractedText.length,
            chunkCount: processingResults.chunkCount,
        ...
        logger.warn(" Original document record not found for update", {
          documentId,
  ...
  async close(): Promise<void> {
    try {
      ...
      logger.error("Failed to close vector storage connection", {
        error: error.message,
      });
    }
  }
