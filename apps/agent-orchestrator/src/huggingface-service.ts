...
export class HuggingFaceService {
   ...
   */
  async generateResponse(prompt: string, maxLength: number = 150): Promise<string> {
    try {
      ...
      return result.generated_text?.replace(prompt, '').trim() || 'Unable to generate response';
    } catch (error) {
      ...
      try {
        ...
        return fallbackResult.generated_text?.replace(prompt, '').trim() || 'Unable to generate response';
      } catch {
   ...
   * Summarize documents
   */
  async summarizeDocument(text: string, maxLength: number = 150): Promise<string> {
    try {
  ...
  async processDocumentLayout(documentImage: Buffer): Promise<{
    ...
    try {
      ...
      console.warn('Document layout processing not yet implemented - requires LayoutLMv3 setup');
