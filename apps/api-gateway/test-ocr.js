  ...
  try {
    ...
    try {
      const healthResponse = await axios.get(
        `${API_BASE_URL}/debug/ocr/system-check`,
      );
      console.log("System Check Results:");
      console.log(
        `   - Overall Status: ${healthResponse.data.summary.overallStatus}`,
      );
      console.log(
        `   - Tesseract Available: ${healthResponse.data.summary.tesseractAvailable}`,
      );
      console.log(
        `   - Temp Dir Working: ${healthResponse.data.summary.tempDirWorking}`,
      );
      console.log(
        `   - PDF Parse Working: ${healthResponse.data.summary.pdfParseWorking}`,
      );
    ...
    try {
      ...
      const compareResponse = await axios.post(
        `${API_BASE_URL}/debug/ocr/compare-services`,
        {
      ...
      console.log(
        `   - Recommended Service: ${compareResponse.data.comparison.recommendedService}`,
      );
      console.log(
        `   - Performance Difference: ${compareResponse.data.comparison.performanceDifference}ms`,
      );
      console.log(
        `   - Quality Comparison: ${compareResponse.data.comparison.qualityComparison}`,
      );
      console.log(
        `   - Improvement Areas: ${compareResponse.data.comparison.improvementAreas.length}`,
      );
      ...
      console.log(
        `     • Method: ${compareResponse.data.original.extractionMethod}`,
      );
      console.log(`     • Quality: ${compareResponse.data.original.quality}`);
      console.log(
        `     • Text Length: ${compareResponse.data.original.extractedLength}`,
      );

      console.log(" - Enhanced Service:");
      console.log(
        `     • Method: ${compareResponse.data.enhanced.extractionMethod}`,
      );
      console.log(`     • Quality: ${compareResponse.data.enhanced.quality}`);
      console.log(
        `     • Text Length: ${compareResponse.data.enhanced.extractedLength}`,
      );
    } catch (error) {
      console.log(
    ...
    try {
      ...
      const ocrTestResponse = await axios.post(
        `${API_BASE_URL}/debug/ocr/test-ocr-with-force`,
        {
      ...
      console.log(
        `   - Extraction Method: ${ocrTestResponse.data.result.extractionMethod}`,
      );
      console.log(`   - Quality: ${ocrTestResponse.data.result.quality}`);
      console.log(
        `   - Confidence: ${ocrTestResponse.data.result.confidence.toFixed(3)}`,
      );
      console.log(
        `   - Processing Time: ${ocrTestResponse.data.result.processingTime}ms`,
      );
      console.log(
        `   - Text Extracted: ${ocrTestResponse.data.result.extractedLength} characters`,
      );
...
async function testOriginalDebugEndpoint() {
  ...
  try {
    ...
    const originalResponse = await axios.post(
      `${API_BASE_URL}/debug/extract-pdf-text`,
      {
    ...
    console.log(
      `   - Confidence: ${originalResponse.data.confidence.toFixed(3)}`,
    );
    console.log(
      `   - Processing Time: ${originalResponse.data.processingTime}ms`,
    );
    console.log(
      `   - Text Length: ${originalResponse.data.extractedLength} characters`,
    );

    if (
      originalResponse.data.extractedText &&
      originalResponse.data.extractedLength > 0
    ) {
      console.log(
        `   - Preview: "${originalResponse.data.extractedText.slice(0, 100)}..."`,
      );
    }
  } catch (error) {
    console.log(
      "Original debug endpoint failed:",
      error.response?.data || error.message,
    );
  }
}
...
async function main() {
  ...
  console.log(
    "This script will test the OCR system improvements and diagnostics.\n",
  );
