...
async function testIntegration() {
  ...
  try {
    ...
    try {
      ...
      if (response.data.extractedText && (response.data.extractedLength || response.data.extractedText.length) > 0) {
        console.log('   🎉 SUCCESS! ApexFlow is now extracting text properly!');
        console.log(`   - Preview: "${response.data.extractedText.slice(0, 200)}..."`);
      } else {
        console.log('   ⚠️ Still no text extracted - may need to integrate the Docker OCR client');
      }

    } catch (error) {
      console.log('❌ Debug endpoint test failed:', error.response?.data || error.message);
    }
    ...
    try {
      // ApexFlow result
      const apexFlowResponse = await axios.post('http://localhost:3000/debug/extract-pdf-text', {
      ...
      // Docker OCR result  
      const dockerOCRResponse = await axios.post('http://localhost:3001/ocr/process', {
      ...
      console.log(`   - Text Length: ${apexFlowResponse.data.extractedLength || 0} chars`);
      console.log(`   - Quality: ${apexFlowResponse.data.quality}`);
      console.log(`   - Method: ${apexFlowResponse.data.extractionMethod || apexFlowResponse.data.method}`);
      
      console.log('Docker OCR (Target):');
      console.log(`   - Text Length: ${dockerOCRResponse.data.extractedLength} chars`);
      console.log(`   - Quality: ${dockerOCRResponse.data.quality}`);
      console.log(`   - Method: ${dockerOCRResponse.data.extractionMethod}`);

      const improvement = dockerOCRResponse.data.extractedLength - (apexFlowResponse.data.extractedLength || 0);
      if (improvement > 0) {
...
async function showIntegrationSteps() {
  ...
  console.log('2. ✅ Verified OCR works with your free-invoice.pdf (349 chars extracted!)');
  console.log('3. ✅ Created OCR client service for ApexFlow integration');
  console.log('');
  console.log('🔧 TODO (Quick Integration):');
  console.log('1. Update debug.controller.ts to use OcrClientService instead of OcrService');
  console.log('2. Add OcrClientService to debug.module.ts providers');
  console.log('3. Add axios dependency: npm install axios');
  console.log('4. Set environment variable: OCR_SERVICE_URL=http://localhost:3001');
  console.log('');
  ...
  console.log('- Use the Docker service directly: POST http://localhost:3001/ocr/process');
...
async function main() {
  ...
  console.log('  -d \'{"filename": "my-document.pdf", "fileContent": "JVBERi0xLjEK...", "options": {"useOCR": "auto"}}\'');
  console.log('');
}
