  ...
  try {
    ...
    try {
      ...
      if (
        ...
        response.data.extractedText.length > 0
      ) {
        console.log(
      ...
      if (
        response.data.pageBreakdown &&
        response.data.pageBreakdown.length > 0
      ) {
        console.log(" - Page breakdown:");
        response.data.pageBreakdown.forEach((page) => {
          console.log(
            `     Page ${page.pageNumber}: ${page.textLength} chars, confidence: ${page.confidence}, method: ${page.extractionMethod}`,
          );
    ...
    try {
      ...
      if (
        ...
        console.log(
          `   - Preview: "${response.data.extractedText.slice(0, 300)}..."`,
        );
        console.log(
          `   ✅ PDF has extractable text! OCR might not be necessary.`,
        );
      } else {
        console.log(`   - No text extracted via direct parsing`);
        console.log(
          `   ⚠️ PDF likely contains images/scanned content - OCR would be needed`,
        );
      }
    } catch (error) {
...
async function testEnhancedService() {
  ...
  try {
    ...
    const response = await axios.post(
      "http://localhost:3000/debug/ocr/test-ocr-with-force",
      {
        filename: "free-invoice.pdf",
    ...
    console.log(
      `   - Text Length: ${response.data.result.extractedLength} characters`,
    );
    ...
    if (error.response && error.response.status === 404) {
      console.log(
        "ℹ️  Enhanced service not available (service needs restart to load new endpoints)",
      );
    } else {
...
async function main() {
  ...
  console.log(
    "2. If no text extracted, OCR is needed but requires ImageMagick/GraphicsMagick",
  );
  console.log(
    "3. The root issue is missing convert/gm binaries for PDF-to-image conversion",
  );
  console.log(
    "4. Consider alternative solutions if system dependencies can't be installed",
  );
}
