...
} from "../response/document-response.service";
...
export class DocumentUploadOrchestrationService {
  ...
  async handleFileUpload(
    ...
    try {
      ...
      const { workflowSelection, execution } =
        await this.handleWorkflowProcessing(document, options);
  ...
  async handleTestUpload(
    ...
    try {
      ...
      if (body.aiAnalysis) {
        this.logger.log(
          `💾 Saving AI analysis data for document: ${document.id}`,
        );
        ...
        this.logger.log(
          `✅ AI analysis data saved for document: ${document.id}`,
        );
      }
      ...
      if (body.requiresApproval && body.approvalMethod === "slack") {
        this.logger.log(
          `📋 Document requires Slack approval, creating approval request instead of direct processing`,
        );
        ...
        return this.documentResponseService.formatUploadResponse(
          document,
          {
            ...
            confidence: 1.0,
            reason: "Approval workflow selected",
          },
          {
            id: approvalResult.approvalId,
        ...
        const { workflowSelection, execution } =
          await this.handleWorkflowProcessing(document, body);
  ...
  private async handleWorkflowProcessing(
    ...
    if (workflowSelection.workflowId) {
      ...
      execution = await this.executeWorkflowSafely(
        document.id,
  ...
  private async executeWorkflowSafely(
    ...
    try {
      this.logger.log(
        `🚀 Enqueueing document processing job for document ${documentId}, workflow ${workflowId}`,
      );
      ...
      this.logger.log(
        `✅ Document processing job queued successfully: ${jobId}`,
      );
      ...
      this.logger.error(
        `Failed to enqueue document processing job for ${documentId}:`,
        queueError.message,
      );
      ...
      try {
        this.logger.warn(
          `Falling back to direct workflow execution for document ${documentId}`,
        );
        ...
        this.logger.log(
          `✅ Direct workflow execution started successfully for document ${documentId}`,
        );
        return {
        ...
        this.logger.error(
          `Both queue and direct execution failed for document ${documentId}:`,
          directError.message,
        );
        ...
        try {
          this.logger.warn(
            `Attempting direct AI analysis for document ${documentId} as final fallback`,
          );
          ...
          this.logger.error(
            `All processing methods failed for document ${documentId}:`,
            aiError.message,
          );
          return {
  ...
  private async performDirectAIAnalysis(documentId: string): Promise<void> {
    try {
      ...
      const { fileStream } =
        await this.documentsService.getDocumentFile(documentId);
      ...
      const documentAnalyzer =
        this.workflowExecutionService["documentAnalyzerService"];
      if (documentAnalyzer) {
        const aiResult = await documentAnalyzer.analyzeDocument(
          document,
          fileBuffer,
        );
        ...
        this.logger.warn(
          "Document analyzer service not available for direct AI analysis",
        );
      }

      this.logger.log(
        `✅ Direct AI analysis completed for document ${documentId}`,
      );
    } catch (error) {
      this.logger.error(
        `Direct AI analysis failed for document ${documentId}:`,
        error.message,
      );
      throw error;
    }
  ...
  private createSimulatedFile(body: TestUploadBody): any {
    if (!body.originalName) {
      throw new BadRequestException(
        "originalName is required for document upload",
      );
    }
    ...
    if (body.content && body.content !== "simulated file content") {
      try {
        ...
        this.logger.log(
          `📄 Decoded base64 content: ${fileBuffer.length} bytes`,
        );
      } catch (error) {
        this.logger.warn(
          "Failed to decode base64 content, using as plain text",
        );
  ...
  private async createApprovalRequest(
    ...
    try {
      ...
      this.logger.log(
        `🚀 Creating approval request for document: ${document.originalName}`,
      );

      const approval =
        await this.workflowApprovalService.createApproval(approvalData);
      ...
      this.logger.error(
        `  Failed to create approval request for document ${document.id}:`,
        error.message,
      );
      throw error;
    }
   ...
   */
  private async sendSlackApprovalMessage(
    approval: any,
    body: TestUploadBody,
  ): Promise<void> {
    try {
      ...
      const botToken =
        ...
        "xoxb-9491771770549-9500207067301-H1jrmpZVT5SGzYwWevoz2K6y";
      const channel = "#general"; // Use the same channel as the working test

      this.logger.log(
        `🔔 Sending Slack approval message for: ${approval.title}`,
      );
      ...
      const slackMessage: SlackApprovalMessage = {
        ...
        workflowName:
          body.aiAnalysis?.workflowName || "Document Approval Workflow",
      ...
      if (result.success) {
        ...
        this.logger.warn(
          `⚠️ Failed to send Slack message for approval ${approval.id}: ${result.error}`,
        );
      }
    } catch (error) {
      this.logger.error(
        `  Failed to send Slack approval message for ${approval.id}:`,
        error.message,
      );
  ...
  validateUploadRequest(file?: Express.Multer.File, body?: any): void {
    if (!file && !body?.originalName) {
      throw new BadRequestException(
        "No file uploaded and no test data provided",
      );
    }
    ...
    if (file && file.size > 100 * 1024 * 1024) {
      // 100MB limit
      throw new BadRequestException(
        "File size exceeds maximum allowed size (100MB)",
      );
    }
  }
}
