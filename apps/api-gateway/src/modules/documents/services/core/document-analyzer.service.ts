...
export class DocumentAnalyzerService {
  ...
  async analyzeInvoice(request: {
    ...
    try {
      ...
      this.logger.log(
        `✅ Hugging Face AI invoice analysis completed: ${request.filename}`,
      );
      ...
      return {
        ...
        dates: {
          invoiceDate:
            huggingFaceResult.structuredFields.date_info?.invoice_date,
          dueDate: huggingFaceResult.structuredFields.date_info?.due_date,
          serviceDate:
            huggingFaceResult.structuredFields.date_info?.service_date,
        },
        amounts: {
        ...
        lineItems:
          huggingFaceResult.structuredFields.line_items?.map((item) => ({
        ...
        bankDetails:
          huggingFaceResult.structuredFields.payment_info?.bank_details,
   ...
   */
  private convertHuggingFaceResponse(
    ...
    return {
      ...
      metadata: {
        ...
        creationDate: document.createdAt.toISOString(),
        extractionMethod:
          huggingFaceResult.extractionMethod || "Hugging Face AI Service",
      },
      entities: {
        person: {
        ...
        organization: {
          companies: huggingFaceResult.structuredFields.vendor_info?.name
            ? [huggingFaceResult.structuredFields.vendor_info.name]
            : [],
          addresses: huggingFaceResult.structuredFields.vendor_info?.address
            ? [huggingFaceResult.structuredFields.vendor_info.address]
            : [],
        },
        financial: {
          amounts: huggingFaceResult.structuredFields.financial_info?.total
            ? [
                huggingFaceResult.structuredFields.financial_info.total.toString(),
              ]
            : [],
          currencies: huggingFaceResult.structuredFields.financial_info
            ?.currency
            ? [huggingFaceResult.structuredFields.financial_info.currency]
            : [],
          accountNumbers: [],
          invoiceNumbers: huggingFaceResult.structuredFields.invoice_number
            ? [huggingFaceResult.structuredFields.invoice_number]
            : [],
          dates: huggingFaceResult.structuredFields.date_info?.invoice_date
            ? [huggingFaceResult.structuredFields.date_info.invoice_date]
            : [],
        },
        dates: huggingFaceResult.structuredFields.date_info
          ? Object.values(huggingFaceResult.structuredFields.date_info)
              .filter(Boolean)
              .map(String)
          : [],
        locations: huggingFaceResult.structuredFields.vendor_info?.address
          ? [huggingFaceResult.structuredFields.vendor_info.address]
          : [],
      },
      structuredFields: {
      ...
      summary:
        huggingFaceResult.metadata?.extractionSummary ||
        `Document processed by Hugging Face AI - ${huggingFaceResult.metadata?.fieldsFound || 0} fields found`,
