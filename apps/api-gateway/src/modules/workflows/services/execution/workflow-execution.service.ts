...
export class WorkflowExecutionService {
  ...
  async executeWorkflow(documentId: string, workflowId: string): Promise<void> {
    this.logger.log(
      `Starting workflow execution for document ${documentId}, workflow ${workflowId}`,
    );
    ...
    try {
      ...
      for (const step of enabledSteps) {
        ...
        const currentDoc = await this.documentsRepository.findOne({
          where: { _id: documentId as any },
        });
        const updatedSteps = [
          ...(currentDoc.workflowExecution.steps || []),
          stepResult,
        ];
      ...
      this.logger.log(
        `Workflow execution completed for document ${documentId}`,
      );
    } catch (error) {
      this.logger.error(
      ...
      switch (step.type) {
        ...
        case "analyze_content":
          ...
          const workflow = await this.workflowService.getWorkflowById(
            document.workflowExecution.workflowId,
          );
  ...
  private async extractText(document: Document): Promise<any> {
    this.logger.log(
      `🔍 Starting text extraction for document: ${document.originalName}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `📄 Processing document: ${document.originalName} (${fileBuffer.length} bytes)`,
      );
      ...
      this.logger.log(
        `✅ Text extraction completed for ${document.originalName}`,
      );
      this.logger.log(
        `   Method: ${method} | Confidence: ${Math.round(confidence * 100)}% | Text length: ${extractedText.length} chars | Time: ${processingTime}ms`,
      );
      ...
      this.logger.error(
        `  Text extraction failed for ${document.originalName}:`,
        error.message,
      );
  ...
  private async analyzeContent(
    document: Document,
    workflowContext?: { workflow?: Workflow; step?: WorkflowStep },
  ): Promise<any> {
    this.logger.log(
      `🚀 Starting Visual AI content analysis for ${document.originalName}`,
    );
    ...
    try {
      ...
      if (!extractedText) {
        throw new Error(
          "No extracted text found. Text extraction must run before analysis.",
        );
      }

      this.logger.log(
        `📄 Analyzing ${extractedText.length} characters with Visual AI`,
      );
      ...
      const fileContent = Buffer.from(extractedText, "utf-8").toString(
        "base64",
      );
      const analyzerResult = await this.huggingFaceClient.analyzeDocument({
        fileContent: fileContent,
      ...
      this.logger.error(
        `  Visual AI analysis failed for ${document.originalName}:`,
        error,
      );
  ...
  private async sendNotification(
    ...
    if (integrations.length === 0) {
      this.logger.warn(
        `No active ${config.integrationType} integrations found`,
      );
  ...
  private async requireApproval(
    ...
    this.logger.log(
      `⏸️  Workflow paused for approval: ${document.originalName}`,
    );
    ...
    try {
      ...
      const approvalRequest = {
        ...
        stepName: stepName,
        approvalType:
          (config.approvalType as ApprovalType) || ApprovalType.WORKFLOW_STEP,
        ...
        description:
          config.description ||
          `Manual approval is required to continue processing ${document.originalName}`,
        metadata: {
      ...
      const approval =
        await this.workflowApprovalService.createApproval(approvalRequest);
      ...
      this.logger.log(
        `✅ Approval request created for ${document.originalName} (${approval.id})`,
      );
      ...
      this.logger.error(
        `  Failed to create approval request for ${document.originalName}:`,
        error,
      );
  ...
  async getDocumentWithAnalysis(documentId: string): Promise<Document | null> {
    try {
      ...
      const objectId = ObjectId.isValid(documentId)
        ? new ObjectId(documentId)
        : documentId;

      this.logger.log(`🔍 Querying for document with analysis: ${documentId}`);
      this.logger.log(
        `🔍 ObjectId valid: ${ObjectId.isValid(documentId)}, converted: ${objectId}`,
      );
      ...
      if (document) {
        ...
        this.logger.log(
          `🔍 Processing results: ${!!document.processingResults}`,
        );
        this.logger.log(
          `🔍 Workflow execution: ${!!document.workflowExecution}`,
        );
      }
  ...
  private async getPdfBuffer(document: Document): Promise<Buffer | null> {
    try {
      if (!document.fileId) {
        this.logger.warn(
          `No file ID found for document: ${document.originalName}`,
        );
        return null;
      }
      ...
      if (!document.mimeType?.includes("pdf")) {
        this.logger.log(
          `Document ${document.originalName} is not a PDF (${document.mimeType}), skipping PDF processing`,
        );
        return null;
      }

      this.logger.log(
        `📥 Retrieving PDF buffer for Visual AI processing: ${document.originalName}`,
      );
      ...
      this.logger.warn(
        `⚠️ Failed to get PDF buffer for ${document.originalName}: ${error.message}`,
      );
      return null;
    }
  }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        stream.on("error", (error) => {
          this.logger.error(
            `Stream error while downloading file ${fileId}:`,
            error,
          );
