...
export class SlackMessagingService {
  ...
  async sendApprovalMessage(
    ...
    try {
      ...
      if (
        ...
        const extractedFields = this.formatExtractedDataForSlack(
          message.extractedData,
        );
      ...
      if (response.data.ok) {
        this.logger.log(
          `✅ Slack approval message sent successfully to ${channel}`,
        );
  ...
  async updateApprovalMessage(
    ...
    try {
      ...
      if (response.data.ok) {
        ...
        this.logger.error(
          `  Failed to update Slack message: ${response.data.error}`,
        );
        return {
          success: false,
          error: response.data.error,
  ...
  private formatExtractedDataForSlack(
    extractedData: Record<string, any>,
  ): string {
    const fields: string[] = [];
