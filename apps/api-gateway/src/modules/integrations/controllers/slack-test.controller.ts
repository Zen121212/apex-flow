...
import { SlackMessagingService, SlackApprovalMessage } from '../services/slack-messaging.service';
...
export class SlackTestController {
  ...
  async testSlackApproval(@Body() body: any): Promise<any> {
    try {
      ...
      const slackMessage: SlackApprovalMessage = {
        ...
        description: messageData.description || 'This is a test approval message sent directly from ApexFlow backend!',
        channel: channel
      };

      const result = await this.slackMessagingService.sendApprovalMessage(botToken, slackMessage);

      this.logger.log(`🎯 Test result: ${result.success ? 'SUCCESS' : 'FAILED'}`);
  ...
  async testSlackConnection(@Body() body: any): Promise<any> {
    try {
      ...
      const result = await this.slackMessagingService.testSlackConnection(botToken, channel);
