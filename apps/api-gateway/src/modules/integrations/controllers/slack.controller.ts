import { Controller, Post, Body, Logger, HttpException, HttpStatus, Headers } from '@nestjs/common';
import { WorkflowApprovalService, ApprovalDecision } from '../../workflows/services/approval/workflow-approval.service';
...
export class SlackController {
  ...
  private async handleButtonClick(payload: any): Promise<any> {
    try {
      ...
      this.logger.log(`🔘 Button clicked: ${action.action_id} by ${user.name || user.id}`);
      
      if (action.action_id === 'approve_button' || action.action_id === 'reject_button') {
        const approvalId = action.value;
        const decision = action.action_id === 'approve_button' ? 'approve' : 'reject';
