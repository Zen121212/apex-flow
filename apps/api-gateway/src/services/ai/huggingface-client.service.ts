...
export class HuggingFaceClientService {
  ...
  async analyzeDocument(
    ...
    try {
      ...
      this.logger.log(
        `📄 Text extracted via ${extractionResult.source}: ${extractionResult.text.length} chars`,
      );

      // Classify document type
      const classification = await this.modelManager.classifyDocument(
      ...
      const structuredFields =
        await this.fieldExtractor.extractStructuredFields(
          extractionResult.text,
          classification.type,
          entities,
        );
      ...
      const fieldCount =
        this.fieldExtractor.countExtractedFields(structuredFields);
      ...
      this.logger.log(
        `✅ Local AI analysis completed for: ${request.fileName}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
  ...
  async analyzeBatch(
    ...
    try {
      this.logger.log(
        `🔄 Starting batch analysis for ${requests.length} documents`,
      );
      ...
      this.logger.log(
        `✅ Batch analysis completed for ${requests.length} documents`,
      );
      return results;
    } catch (error) {
  ...
  async healthCheck(): Promise<any> {
    try {
      ...
      const hasWorkingModels =
        modelHealth.ocr.loaded ||
