...
export class TextExtractorService {
    ...
    try {
      ...
      if (mimeType === "application/pdf") {
        ...
        result = {
          ...
          stats: {
            length: text.length,
            printableRatio: this.printableRatio(text),
          },
        };
      } else {
        this.logger.warn(
          `Unsupported MIME type: ${mimeType}, attempting text extraction`,
        );
        const text = Buffer.from(fileContent, "base64").toString("utf-8");
        result = {
          text,
      ...
      this.logger.log(
        `✅ Text extraction completed: ${result.source}, ${result.text.length} chars`,
      );
      return result;
    } catch (error) {
      this.logger.warn(
  ...
  private async extractFromPDF(
    ...
    try {
      ...
      this.logger.log(
        `📄 Processing PDF: ${fileName}, size: ${buffer.length} bytes`,
      );

      // Check for PDF corruption indicators
      const corruptionLevel = this.detectPDFCorruption(buffer);
      this.logger.log(
        `📄 PDF corruption level: ${corruptionLevel}/10 for ${fileName}`,
      );
      ...
      this.logger.log(
        `📋 PDF parse result: ${data.text ? data.text.length : 0} characters extracted`,
      );
      ...
      if (data.text && data.text.trim().length > 0) {
        ...
        if (
          ...
          this.isLikelyJunk(cleanedText) === false
        ) {
          const printableRatio = this.printableRatio(cleanedText);
          this.logger.log(
            `✅ PDF text extracted successfully: ${cleanedText.length} characters`,
          );
          this.logger.log(
          ...
          this.logger.warn(
            `⚠️ Extracted text failed validation: length=${cleanedText.length}, isJunk=${this.isLikelyJunk(cleanedText)}`,
          );
        }
      } else {
        this.logger.warn("No text extracted from PDF or empty result");
      }

      // If no text found or corruption is high, try OCR
      if (corruptionLevel >= 5 || !data.text || data.text.trim().length === 0) {
        this.logger.log(
          "High corruption or no text found, attempting OCR on scanned pages...",
        );
      ...
      return await this.extractWithAlternativeMethods(
        buffer,
        fileName,
        ocrPipeline,
      );
      ...
      return await this.tryMultipleFallbacks(
        fileContent,
        fileName,
        error.message,
      );
  ...
  private async extractFromScannedPDF(
    ...
    try {
      ...
      this.logger.warn(
        "OCR processing not available for scanned PDFs in Docker environment",
      );
      return {
        text: fallbackText,
        source: "fallback",
      ...
      this.logger.error(
        `  Scanned PDF fallback processing failed: ${error.message}`,
      );
      return {
  ...
  private detectPDFCorruption(buffer: Buffer): number {
    ...
    try {
      ...
      const binaryRatio =
        (content.match(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g) || []).length /
        content.length;
      if (binaryRatio > 0.1)
        corruptionScore += Math.min(3, Math.floor(binaryRatio * 10));
      ...
      for (const strategy of strategies) {
        try {
          ...
          if (data.text && data.text.trim().length > 0) {
            const cleanedText = this.cleanExtractedText(data.text);
            if (
              cleanedText.length > CONFIG.MIN_TEXT_LENGTH &&
              !this.isLikelyJunk(cleanedText)
            ) {
              const printableRatio = this.printableRatio(cleanedText);
              this.logger.log(
                `✅ Alternative PDF extraction successful: ${cleanedText.length} characters`,
              );
              return {
                text: cleanedText,
                source: "pdf-parse",
  ...
  private async extractFromImage(
    ...
    try {
      ...
      if (text && text.trim().length > 0) {
        ...
        this.logger.log(
          `✅ Image OCR completed: ${cleanedText.length} characters`,
        );
        return {
          text: cleanedText,
          source: "ocr",
  ...
  private async tryMultipleFallbacks(
    ...
    this.logger.log(
      `🔄 Attempting fallback extraction methods for ${fileName}`,
    );
    ...
    for (let i = 0; i < fallbacks.length; i++) {
      try {
        this.logger.log(
          `🔄 Trying fallback method ${i + 1}/${fallbacks.length}`,
        );
        ...
        if (result && result.length > 20 && !this.isLikelyJunk(result)) {
          ...
          if (!this.looksLikeBase64(result)) {
            ...
            this.logger.log(
              `✅ Fallback extraction successful with method ${i + 1}`,
            );
            this.logger.log(
              `📝 Fallback result preview: ${result.substring(0, 100)}`,
            );
            return {
              text: result,
              source: "fallback",
            ...
            this.logger.warn(
              `⚠️ Fallback method ${i + 1} returned base64-like content, skipping`,
            );
          }
        } else {
          this.logger.debug(
            `⚠️ Fallback method ${i + 1} returned insufficient or junk text`,
          );
        }
      } catch (fallbackError) {
        this.logger.debug(
          `  Fallback method ${i + 1} failed: ${fallbackError.message}`,
        );
      }
    }
  ...
  private async tryPartialTextExtraction(fileContent: string): Promise<string> {
    ...
    const textMarkers = [/BT\s+(.*?)\s+ET/g, /\(([^)]+)\)/g, /\[([^\]]+)\]/g];
