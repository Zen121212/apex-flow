...
export interface QueryAnalysis {
  intent: 'search' | 'find_highest' | 'find_lowest' | 'compare' | 'summary' | 'count';
  entityType: 'invoice' | 'contract' | 'receipt' | 'document';
  searchCriteria: {
    field?: string; // e.g., 'price', 'amount', 'total', 'date'
    operator?: 'highest' | 'lowest' | 'equal' | 'greater' | 'less' | 'between' | 'contains';
    value?: string | number;
...
class AIDocumentSearchService {
  ...
  async search(query: string): Promise<DocumentSearchResult[]> {
    try {
      ...
      if ((queryAnalysis.intent === 'find_highest' || queryAnalysis.intent === 'find_lowest') && this.documentCache.size > 0) {
  ...
  private async analyzeQuery(query: string): Promise<QueryAnalysis> {
    ...
    if (lowerQuery.includes('highest') || lowerQuery.includes('maximum') || lowerQuery.includes('most expensive') || lowerQuery.includes('largest')) {
      intent = 'find_highest';
    } else if (lowerQuery.includes('lowest') || lowerQuery.includes('minimum') || lowerQuery.includes('cheapest') || lowerQuery.includes('smallest')) {
      intent = 'find_lowest';
      ...
      intent = 'summary';
    } else if (lowerQuery.includes('how many') || lowerQuery.includes('count') || lowerQuery.includes('what') && (lowerQuery.includes('do i have') || lowerQuery.includes('are there'))) {
      intent = 'count';
    }
    ...
    if (lowerQuery.includes('price') || lowerQuery.includes('amount') || lowerQuery.includes('cost') || lowerQuery.includes('total')) {
      field = 'amount';
    ...
    } else if (lowerQuery.includes('this year') || lowerQuery.includes('2024')) {
   ...
   */
  private async executeSearch(queryAnalysis: QueryAnalysis, originalQuery: string): Promise<DocumentSearchResult[]> {
    ...
    if (queryAnalysis.filters.documentType) {
      ...
      filteredDocs = filteredDocs.filter(doc => 
        doc.documentType.toLowerCase().includes(queryAnalysis.filters.documentType!)
      );
      console.log(`📑 Document type filter (${queryAnalysis.filters.documentType}): ${beforeCount} → ${filteredDocs.length}`);
      filteredDocs.forEach(doc => console.log(`   - ${doc.fileName} (type: ${doc.documentType})`));
    }
    ...
    switch (queryAnalysis.intent) {
      case 'find_highest':
      case 'find_lowest':
        results = this.findByAmount(filteredDocs, queryAnalysis.intent === 'find_highest', originalQuery);
        break;
      
      case 'count':
        results = this.listDocuments(filteredDocs, originalQuery, queryAnalysis);
        break;
   ...
   */
  private findByAmount(documents: DocumentIndex[], findHighest: boolean, query: string): DocumentSearchResult[] {
    ...
    documents.forEach(doc => {
      ...
      if (extractedData?.financial_info?.total) {
        amount = typeof extractedData.financial_info.total === 'number' 
          ? extractedData.financial_info.total
          : parseFloat(String(extractedData.financial_info.total).replace(/[^0-9.-]/g, ''));
      }
      
      // Fallback to other amount fields
      if (!amount && extractedData?.total) {
        amount = typeof extractedData.total === 'number'
          ? extractedData.total
      ...
      if (!amount) {
        const amountField = doc.extractedData.searchableFields.find(field => 
          ['amount', 'total', 'price', 'cost'].includes(field.name.toLowerCase()) && 
    ...
    // Sort by amount
    documentsWithAmounts.sort((a, b) => findHighest ? b.amount - a.amount : a.amount - b.amount);
    ...
    return documentsWithAmounts.map(({ doc, amount }, index) => ({
      ...
      queryAnalysis: {
        ...
        entityType: 'invoice',
        searchCriteria: { field: 'amount', operator: findHighest ? 'highest' : 'lowest' },
        filters: {}
      }
    }));
  }
   ...
   */
  private listDocuments(documents: DocumentIndex[], query: string, queryAnalysis: QueryAnalysis): DocumentSearchResult[] {
    const results: DocumentSearchResult[] = documents.map((doc, index) => {
      ...
      if (doc.extractedData.structuredFields?.financial_info?.total) {
        ...
        const currency = doc.extractedData.structuredFields.financial_info.currency || 'USD';
   ...
   */
  private performTextSearch(documents: DocumentIndex[], query: string): DocumentSearchResult[] {
  ...
  private generateTags(doc: DocumentIndex): string[] {
    ...
    // Add date-based tags
    const uploadMonth = doc.uploadDate.toLocaleString('default', { month: 'long' }).toLowerCase();
  ...
  private async refreshDocumentIndex(): Promise<void> {
    try {
      ...
      for (const doc of documents) {
        if (!this.documentCache.has(doc.id)) {
          try {
            ...
            const searchableFields = this.createSearchableFields(analysisResult);
            
            // Cache with real analyzed data
            this.documentCache.set(doc.id, {
              documentId: doc.id,
              fileName: doc.originalName,
              documentType: analysisResult.documentType || this.determineAnalysisType(doc.originalName),
            ...
            console.log(`✅ Indexed document: ${doc.originalName} with real data`);
          } catch (docError) {
            console.warn(`⚠️ Failed to index document ${doc.originalName}:`, docError);
      ...
      console.log(`📊 Indexing complete! Cached ${this.documentCache.size} documents:`);
      Array.from(this.documentCache.values()).forEach((doc, index) => {
        console.log(`   ${index + 1}. ${doc.fileName} (type: ${doc.documentType})`);
      });
    } catch (error) {
   ...
   */
  private createSearchableFields(analysisResult: HuggingFaceAnalysisResult): SearchableField[] {
    const fields: SearchableField[] = [];
    
    // Add structured fields as searchable
    if (analysisResult.structuredFields) {
      Object.entries(analysisResult.structuredFields).forEach(([key, value]) => {
   ...
   */
  private determineFieldType(value: any): 'text' | 'number' | 'date' | 'currency' {
   ...
   */
  private determineAnalysisType(fileName: string): 'invoice' | 'contract' | 'receipt' | 'general' {
